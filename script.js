...
window.addEventListener("scroll", function(event) {
  ...
  var fixThresh7 = fixThresh * 7 + unfixHeight * 6;
  ...
  var seventhExit = fixThresh6 + unfixHeight + halfUnfix;
  var eighthEnter = fixThresh7 + halfUnfix;
  var eighthExit = fixThresh7 + unfixHeight + halfUnfix;

  // Fixing/Unfixing States

  // First Fix Enter
  if (scrollHeight <= firstExit) {
    document.querySelector(".title").innerHTML = "strongbox";
    container.style.position = "fixed";
    container.style.left = "0";
    container.style.right = "0";
  ...
  if (scrollHeight >= secondEnter && scrollHeight <= secondExit) {
    document.querySelector(".title").innerHTML = "cdc centrifuge";
    container.style.position = "fixed";
    container.style.left = "0";
    container.style.right = "0";
  ...
  if (scrollHeight >= sixthEnter && scrollHeight <= sixthExit) {
    ...
    container.style.left = "0";
    container.style.right = "0";
    container.style.top = -(5 * marginHeight + 5 * projectHeight) + "px";

    // Sixth Fix Halfway - Text Fade
    if (scrollHeight < fixThresh5 + unfixHeight) {
      title.style.opacity = (scrollHeight - sixthEnter) / (scrollHeight - sixthEnter + halfUnfix) * 2;
    } else {
      title.style.opacity = 1 - ((scrollHeight - sixthEnter - halfUnfix) / ((scrollHeight - sixthEnter))) * 2;
    }
  }

  // Sixth Fix Exit
  if (scrollHeight > sixthExit && scrollHeight < seventhEnter) {
    container.style.position = "relative";
    container.style.top = (5 * unfixHeight + halfUnfix) + "px";
  }

  // Seventh Fix Enter
  if (scrollHeight >= seventhEnter && scrollHeight <= seventhExit) {
    document.querySelector(".title").innerHTML = "animation and video";
    container.style.position = "fixed";
    container.style.left = "0";
    container.style.right = "0";
    container.style.top = -(6 * marginHeight + 6 * projectHeight) + "px";

    // Seventh Fix Halfway - Text Fade
    if (scrollHeight < fixThresh5 + unfixHeight) {
      title.style.opacity = (scrollHeight - seventhEnter) / (scrollHeight - seventhEnter + halfUnfix) * 2;
    } else {
      title.style.opacity = 1 - ((scrollHeight - seventhEnter - halfUnfix) / ((scrollHeight - seventhEnter))) * 2;
    }
  }

  // Seventh Fix Exit
  if (scrollHeight > seventhExit && scrollHeight < eighthEnter) {
    document.querySelector(".title").innerHTML = "";
    container.style.position = "relative";
    container.style.top = (6 * unfixHeight + halfUnfix) + "px";
    aboutText.style.position = "relative";
    aboutText.style.top = "px";
  }

  // Eighth (Text) Fix Enter
  if (scrollHeight >= eighthEnter && scrollHeight <= eighthExit) {
  ...
  if (scrollHeight > fixThresh7 + marginHeight) {
    textOne.style.opacity = (scrollHeight - eighthEnter + marginHeight) / (scrollHeight - eighthEnter + marginHeight + halfUnfix) * 2;
  } else {
    textOne.style.opacity = 0;
  }

  if (scrollHeight > fixThresh7 + marginHeight + aboutTextScrollDist) {
    textTwo.style.opacity = (scrollHeight - eighthEnter + marginHeight - aboutTextScrollDist) / (scrollHeight - eighthEnter + marginHeight + halfUnfix - aboutTextScrollDist) * 2;
  } else {
    textTwo.style.opacity = 0;
  }

  if (scrollHeight > fixThresh7 + marginHeight + aboutTextScrollDist * 2) {
    textThree.style.opacity = (scrollHeight - eighthEnter + marginHeight - aboutTextScrollDist * 2) / (scrollHeight - eighthEnter + marginHeight + halfUnfix - aboutTextScrollDist * 2) * 2;
  } else {
    textThree.style.opacity = 0;
  }

  if (scrollHeight > fixThresh7 + marginHeight + aboutTextScrollDist * 3) {
    textFour.style.opacity = (scrollHeight - eighthEnter + marginHeight - aboutTextScrollDist * 3) / (scrollHeight - eighthEnter + marginHeight + halfUnfix - aboutTextScrollDist * 3) * 2;
  } else {
    textFour.style.opacity = 0;
  }

  if (scrollHeight > fixThresh7 + marginHeight + aboutTextScrollDist * 4) {
    textFive.style.opacity = (scrollHeight - eighthEnter + marginHeight - aboutTextScrollDist * 4) / (scrollHeight - eighthEnter + marginHeight + halfUnfix - aboutTextScrollDist * 4) * 2;
  } else {
